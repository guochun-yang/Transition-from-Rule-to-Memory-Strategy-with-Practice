function [y,dm,noEnt,intreg, preRT] = ModelCostFunction(x, trueSeqs, probeSeqs, behChoice, behCorrect, behRT, imodel, nreplay0)

[posSelection, taskPrediction, ~, expectedEntropy, simPos, simSteps] = Model(trueSeqs, probeSeqs, x, imodel, nreplay0);
...
if ismember(imodel,[3 4 5 9])%%% %previously only 3 4. 5 was based on the all trial  %%!!
...
elseif imodel == 9
    ...
    simtrial_accum = [];
    for i = 1:size(simSteps,1)
        simtrial_accum(i,1) = length(find(simSteps(1:i)>1)) + nreplay0;
    end
    interaction = zscore(simtrial_accum) .* zscore(simSteps);
    interaction(setdiff(1:240,simulatedtrial),1) = 0;
    intreg = interaction;
